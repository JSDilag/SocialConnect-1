    ...
    <body>
        ...
        %>
         ...
         function EditComment(CommentID)
          {
            if (CommentID != null) 
           <%  {    session.setAttribute("commentID", session.getAttribute("CommentID"));
        ...
        {
         if (PostID != null) 
         <%   { session.setAttribute("postID", session.getAttribute("PostID"));
        ...
        <% 
            ...
            DBConnection.ExecQuery("INSERT INTO Gets VALUES (909906, " + session.getAttribute("PostID") + ")");
        ...
        {
        <% DBConnection.ExecQuery("INSERT INTO LikesPost VALUES (" + session.getAttribute("PostID") +", " + ID+")");
            response.sendRedirect("CustomerHomepage.jsp");
        %>
            alert("Post Liked");
        }
        ...
        {
        <% DBConnection.ExecQuery("INSERT INTO LikesComment VALUES (" + session.getAttribute("CommentID") +", " + ID+")");
            response.sendRedirect("CustomerHomepage.jsp");
        %>
            alert("Comment Liked");
        }
        ...
        {
        <% DBConnection.ExecQuery("delete from LikesPost where PostID=" + session.getAttribute("PostID")+"and CustomerID=" + ID +")");
            response.sendRedirect("CustomerHomepage.jsp");
        %>
            alert("Post Unliked");
        }
        ...
        {
        <% DBConnection.ExecQuery("delete from LikesComment where CommentID=" + session.getAttribute("CommentID") +"and CustomerID=" +  ID+")");
            response.sendRedirect("CustomerHomepage.jsp");
        %>
                        ...
                        <div class="col-xs-6">   <!-- Middle column -->
                            <div class="posts">
                                ...
                                <div class="col-xs-5" style="width: 400px; height: 100px; background-color: yellow; border-style: solid;">
                                    ...
                                    <% if (IDs.equals(rs.getString(4))) //user is Owner of post 
                                        {%> <a onclick="<%session.setAttribute("PostID", rs.getString(1));%>; EditPost(<%=rs.getString(1)%>)">Edit</a>  
                                    <a onclick="<%session.setAttribute("PostID", rs.getString(1));%>; DeletePost(<%=rs.getString(1)%>)">Delete</a>
                                    <% }
                                        ps = DBConnection.ExecQuery("select * from LikesPost P where P.PostID = " + rs.getString(1) + "and P.CustomerID = " + ID);
                                        if (ps.next())
                                        {%> <a onclick="<%session.setAttribute("PostID", rs.getString(1));%>; LikesPost(<%=rs.getString(1)%>)">Like</a>  
                                    <%  }else %> 
                                    <a onclick="<%session.setAttribute("PostID", rs.getString(1));%>; UnlikesPost(<%=rs.getString(1)%>)">Unlike</a> 
                                    <!-- Comments -->
                                    ...
                                    %>    
                                    <a onclick="<%session.setAttribute("CommentID", cs.getString(6));%>; EditComment(<%=cs.getString(6)%>)"> Edit </a>
                                    <% }
                                        if (ps.next())
                                        {%>  <a onclick="<%session.setAttribute("CommentID", cs.getString(1));%>; UnlikesComment(<%=cs.getString(1)%>);"> Unlike </a> 
                                    <%  }
                                        else %>  
                                            <a onclick="<%session.setAttribute("CommentID", cs.getString(1));%>; LikesComment(<%=cs.getString(1)%>)"> Like </a> 
                                    %>
                                    <br>    
                                    </tr>   
                                    <%  }
